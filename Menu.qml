...
Item {
  ...
  // Tightened from Style.spacing.xs (3px) per user preference for a denser list.
  property int rowSpacing: Style.spacing.xxs
  property int dividerHeight: Style.space(17)
  property bool searchDivider: false
  property int layoutSerial: 0
  // Docked width for the main nav menu — a bit wider than the stock 300 so a
  // full-height sidebar doesn't feel narrow. Dmenu popups and the two wide
  // submenus (screenrecord options, font picker) keep their own sizing.
  property int dockCardWidth: Style.space(340)
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : root.dockCardWidth), panel.width - Style.gapsOut * 2)
  property int visibleRowsHeight: root.dmenuActive ? dmenuRowListHeight(layoutSerial, displayModel.count, filterText) : rowListHeight(layoutSerial, displayModel.count, filterText, searchDivider)
  // Horizontal bar thickness, used to dock the main menu just below it.
  // Assumes a top-positioned horizontal bar (this machine's config); adjust
  // if the bar is ever moved to another edge.
  readonly property int barThickness: Style.bar.sizeHorizontal
  property int cardHeight: root.dmenuActive
    ? Math.min(contentMargin * 2 + headerHeight + (mode === "input" ? 0 : contentSpacing + visibleRowsHeight), panel.height - Style.gapsOut * 2)
    : (panel.height - root.barThickness - Style.gapsOut * 2)
  ...
  PanelWindow {
    id: panel
    // Dmenu popups (select/input — power menu, screenshot options, etc.)
    // close instantly like before. The main nav menu lingers mapped for
    ...
    // The dmenu card still opens centered exactly as always. The first
    // search keystroke or submenu move freezes the top line where it
    // currently sits — from then on the card grows and shrinks downward
    // instead of re-centering on every resize, which made the menu jump
    // around. The rows height is frozen at the same moment, so the starting
    // menu also caps how tall the card may grow from there. Closing
    // unfreezes both. The docked main menu ignores all of this and just
    // sits fixed below the bar.
    property int cardTop: -1
    property int maxRowsHeight: -1
    readonly property int dockTop: root.barThickness + Style.gapsOut
    readonly property int centeredTop: Math.max(Style.gapsOut, Math.round((height - root.cardHeight) / 2))
    readonly property int effectiveCardTop: root.dmenuActive ? (cardTop >= 0 ? cardTop : centeredTop) : dockTop
    ...
    BorderSurface {
      ...
      // Dmenu popups stay centered horizontally. The main menu slides in
      // from the left edge and slides back out past it on close.
      x: root.dmenuActive
        ? Math.round((panel.width - width) / 2)
        : (root.opened ? Style.gapsOut : -width - Style.gapsOut)
      Behavior on x {
        enabled: !root.dmenuActive
        NumberAnimation { duration: 190; easing.type: Easing.OutCubic }
      }
      ...
      Column {
        ...
        Item {
          ...
          // the original content-hugging height.
          height: root.dmenuActive
            ? root.visibleRowsHeight
            : (menuColumn.height - headerRow.height - root.contentSpacing * 2)
